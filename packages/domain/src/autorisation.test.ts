...
describe('verifierAutorisation', () => {
  it('autorise un numéro qui a un consentement actif', () => {
    const resultat = verifierAutorisation('06 12 34 56 78', [consentement('+33612345678')], maintenant);

    expect(resultat.autorise).toBe(true);
    if (resultat.autorise) expect(resultat.numero).toBe('+33612345678');
  });

  it('refuse un numéro sans aucun consentement', () => {
    const resultat = verifierAutorisation('+33612345678', [consentement('+33699999999')], maintenant);

    expect(resultat).toEqual({ autorise: false, raison: 'aucun-consentement' });
  });

  it('refuse un numéro dont le consentement a été révoqué', () => {
    const revoque = consentement('+33612345678', { revoqueLe: new Date('2026-09-20T00:00:00Z') });

    const resultat = verifierAutorisation('+33612345678', [revoque], maintenant);
  ...
  it('autorise encore un numéro dont la révocation est programmée plus tard', () => {
    const revoqueDemain = consentement('+33612345678', { revoqueLe: new Date('2026-09-26T00:00:00Z') });

    expect(verifierAutorisation('+33612345678', [revoqueDemain], maintenant).autorise).toBe(true);
  });

  it("refuse un consentement qui n'est pas encore accordé", () => {
    const futur = consentement('+33612345678', { accordeLe: new Date('2026-10-01T00:00:00Z') });

    expect(verifierAutorisation('+33612345678', [futur], maintenant)).toEqual({
      autorise: false,
  ...
  it('autorise si un nouveau consentement suit une révocation', () => {
    const ancien = consentement('+33612345678', { revoqueLe: new Date('2026-09-10T00:00:00Z') });
    const nouveau = consentement('+33612345678', {
      accordeLe: new Date('2026-09-15T00:00:00Z'),
      texteVersion: 2,
    });

    const resultat = verifierAutorisation('+33612345678', [ancien, nouveau], maintenant);
  ...
  it('ne laisse fabriquer un numéro autorisé que par la vérification', () => {
    ...
    expectTypeOf(normaliserNumero('+33612345678')).not.toExtend<NumeroAutorise | null>();
  });
});
